...
plotRecords <- function(records, bsp){
  ...
  for (i in 1:length(records)){
    allRec <- allRec %>% bind_rows(records[[i]] %>% as_tibble %>% mutate(repNum=paste0("R", i), cycle=1:nCycKept + (nCyc - nCycKept)))
  }
  ...
  plotData <- plotData %>% mutate(stage=so[stage])
  bp <- brkdn.plot(meanGenoVal ~ stage + cycle, data=plotData, lwd=2, cex=1, col=order(so), md="std.error", stagger=1/(nCycKept+1)/3/bsp$nStages, xlab="Time Period", ylab="Genetic Improvement", main="")
