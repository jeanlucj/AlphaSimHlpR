...
specifyPipeline <- function(bsp=NULL, ctrlFileName=NULL){
  if (is.null(ctrlFileName)){ # NULL control file: make toy example
    ...
    parmNames <- c("nStages", "stageNames", "nParents", "nCrosses", "nProgeny",
                   "useOptContrib", "nCandOptCont", "breedCycleTime", "targetEffPopSize",
                   "nEntries", "nReps", "nLocs", "nChks", "entryToChkRatio", "errVars", "useCurrentPhenoTrain",
                   "nCyclesToKeepRecords", "selCritPipeAdv", "selCritPopImprov")
  ...
  # alternative would be to fix this for the user in some way
  if((bspNew$nCrosses * bspNew$nProgeny) < bspNew$nEntries[1]){
    print("Not enough F1s to fill up Stage 1 trial. nCrosses * nProgeny must by >= nEntries for first stage.")
    stop()
    }
