...
bsp <- specifyCosts(bsp, ctrlFileName="../inst/CostsControlFile.txt")
print(bsp)
...
## Run a simple breeding scheme for `r nCycles` cycles

Replicate a very simple breeding program `r nReplications` times. 
```{r Replicate Scheme}
replicRecords <- lapply(1:nReplications, runBreedingScheme, nCycles=6, initializeFunc=initFuncADChk, productPipeline=prodPipeFncChk, populationImprovement=popImprov1Cyc, bsp)
```
...
plot(1:nrow(meanMeans), meanMeans[,1], type="l", ylim=ylim, xlab="Cycle", ylab="Pop mean", main=paste("Annual cost:", bsp$totalCosts))
for (i in 2:ncol(meanMeans)) lines(1:nrow(meanMeans), meanMeans[,i], col=i)
legend(1, ylim[2], bsp$stageNames, lwd=2, col=1:bsp$nStages, cex=0.6)
```
